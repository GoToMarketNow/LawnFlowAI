...
function sanitizeMessage(text: string, channel: "sms" | "email"): string {
  let sanitized = text
    .replace(/[ὠ0-ὤF]/gu, "") // Emoticons
    .replace(/[ἰ0-ὔF]/gu, "") // Maps and Geographical
    .replace(/[Ὠ0-ὤF]/gu, "") // Transport and Map
    .replace(/[Ὠ0-ὤF]/gu, "") // Transport and Map
    .replace(/[☀-⛿]/gu, "") // Miscellaneous Symbols
    .replace(/[✀-➿]/gu, "") // Dingbats
    .replace(/[︀-️]/gu, "") // Variation Selectors
    .replace(/[ᾐ0-ᾔF]/gu, "") // Supplemental Symbols and Pictographs
    .replace(/[ᾘ0-ᾙF]/gu, "") // Animal and Nature Supplemental
    .replace(/[ᾧ0-ᾧF]/gu, "") // Symbols and Pictographs Extended-A
    .trim();
...
export async function runInvoiceBuildAgent(
  ...
  const businessConfig = await storage.getBusinessProfile(businessId);
  if (!businessConfig) {
    throw new Error(`Business profile not found for businessId ${businessId}.`);
  }
  const businessName = businessConfig.name;

  const pricingConfig = (await configurationService.getAgentConfig(businessId, "pricing_rules"))?.configJson as PricingRules;
  if (!pricingConfig) {
    ...
    for (const invoice of invoicesForReconciliation) {
      ...
      if (totalPaid > (invoice.total || 0)) {
        issues.push({
          type: "OVERPAYMENT",
          severity: "MED",
          description: `Invoice overpaid by ${formatCents(totalPaid - (invoice.total || 0))}`,
          amount: totalPaid - (invoice.total || 0),
        });
      ...
      if (invoice.status === "SENT" && totalPaid > 0 && totalPaid < (invoice.total || 0)) {
        issues.push({
          type: "VARIANCE",
          severity: "LOW",
...
export async function runBillingAgent(
  ...
  const config = (await configurationService.getAgentConfig(businessId, "billing_agent"))?.configJson as BillingConfig;
  const policy = (await configurationService.getAgentConfig(businessId, "policy_agent"))?.configJson as PolicyThresholds; // Assuming "policy_agent" is the key for policy configs
...
export async function runInvoiceSyncAgent(
  ...
  try {
    ...
    // Initialize client
    const quickbooksConfig = (await configurationService.getAgentConfig(accountId, "quickbooks_config"))?.configJson as QuickBooksConfig;
    if (!quickbooksConfig) {
      throw new Error(`QuickBooks configuration not found for businessId ${accountId}.`);
    }

    const client = new QuickBooksClient(accountId, {
      realmId: quickbooksConfig.realmId || integration.realmId || "",
      accessToken: quickbooksConfig.accessToken || integration.accessToken || "",
      refreshToken: quickbooksConfig.refreshToken || integration.refreshToken || "",
      tokenExpiresAt: quickbooksConfig.tokenExpiresAt || integration.tokenExpiresAt || new Date(),
...
export async function runPaymentSyncAgent(
  ...
  try {
    ...
    const client = new QuickBooksClient(accountId, {
      ...
      tokenExpiresAt: quickbooksConfig.tokenExpiresAt || new Date(),
      baseUrl: quickbooksConfig.baseUrl,
    });
    ...
    // Track new payments to update totals correctly
    ...
    for (const payment of existingPayments) {
      if (payment.invoiceId && (payment.status === "COMPLETED" || payment.status === "SUCCEEDED")) {
        ...
        invoicePaymentTotals.set(payment.invoiceId, current + (payment.amount || 0));
      }
    }
    ...
    for (const invoiceId of invoicesToUpdate) {
      ...
      if (invoice && totalPaid >= (invoice.total || 0)) {
        await storage.updateInvoice(invoiceId, { status: "PAID" });
      } else if (invoice && totalPaid > 0 && totalPaid < (invoice.total || 0)) {
...
export interface RemediationOutput {
  rootCause: "SCOPE_MISநாள்" | "QUALITY_ISSUE" | "PRICING_SURPRISE" | "PAYMENT_ERROR" | "COMMUNICATION_BREAKDOWN" | "UNKNOWN";
