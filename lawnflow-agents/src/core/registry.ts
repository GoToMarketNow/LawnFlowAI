...
import { OnboardingHelpAgent } from '../agents/onboarding-help';
...
export class UnifiedAgentRegistry implements AgentRegistry {
  ...
  private initializeCoreAgents(): void {
    ...
    // Onboarding agents
    this.register(new OnboardingAgent());
    this.register(new OnboardingHelpAgent());
  }
}
  ...
  createAgent(type: string, config?: any): BaseAgent {
    switch (type) {
      ...
      case 'onboarding_help':
        return new OnboardingHelpAgent();
      default:
