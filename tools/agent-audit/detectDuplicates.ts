...
if (!fs.existsSync(classificationPath)) {
  console.error("❌ Error: classification.json not found. Run classifyAgents.ts first.");
  process.exit(1);
}
...
for (const dup of duplicates) {
  const hasAgentService = dup.locations.some((l: any) => l.file.includes("agent-service"));
  const hasServerAgents = dup.locations.some((l: any) => l.file.includes("server/agents") || l.file.includes("server\\agents"));
  const hasLawnflowAgents = dup.locations.some((l: any) => l.file.includes("lawnflow-agents"));
