...
import { db } from "../../db";
import { weatherRiskAssessments } from "../../../shared/schema-weather";
...
// Cache for weather risk during planning
let weatherRiskCache: Map<string, JobWeatherRisk> = new Map();
...
/**
 * Load weather risk data for jobs on a given date
 */
export async function loadWeatherRiskForJobs(
  jobIds: string[],
  planDate: Date
): Promise<Map<string, JobWeatherRisk>> {
  const riskMap = new Map<string, JobWeatherRisk>();
  
  if (jobIds.length === 0) return riskMap;
  ...
  try {
    ...
    // Query recent risk assessments for these jobs
    const assessments = await db
      .select()
      .from(weatherRiskAssessments)
      .where(
        and(
          gte(weatherRiskAssessments.jobStart, startOfDay),
          lte(weatherRiskAssessments.jobStart, endOfDay)
        )
      );

    for (const assessment of assessments) {
      if (assessment.jobId) {
        riskMap.set(assessment.jobId.toString(), {
          jobId: assessment.jobId.toString(),
          riskTier: assessment.riskTier as JobWeatherRisk['riskTier'],
          riskScore: assessment.riskScore,
          primaryDriver: (assessment.drivers as any[])?.[0]?.type
        });
      }
    }

    // Cache for use during planning
    weatherRiskCache = riskMap;
  } catch (error) {
    console.error('[PlanningEngine] Error loading weather risk:', error);
  }
...
/**
 * Get weather risk penalty for job assignment
 * Returns a penalty score (higher = worse)
 */
function getWeatherRiskPenalty(jobId: string): number {
  const risk = weatherRiskCache.get(jobId);
  if (!risk) return 0;

  switch (risk.riskTier) {
    case 'STOP':
      return 1000; // Very high penalty - effectively blocks assignment
    case 'HIGH':
      return 50;   // Significant penalty
    case 'MED':
      return 15;   // Moderate penalty
    case 'LOW':
    default:
      return 0;    // No penalty
  }
...
/**
 * Check if job should be skipped due to weather
 */
function shouldSkipDueToWeather(jobId: string): boolean {
  const risk = weatherRiskCache.get(jobId);
  return risk?.riskTier === 'STOP';
}
...
export function computeDispatchPlan(
  ...
  // Track weather-affected jobs for warnings
  const weatherAffectedJobs: string[] = [];
  const weatherBlockedJobs: string[] = [];

  for (const job of sortedJobs) {
    // Check if job should be skipped due to severe weather
    if (shouldSkipDueToWeather(job.id)) {
      unassignedJobs.push(job.id);
      weatherBlockedJobs.push(job.id);
      continue;
    }
    ...
    // Get weather penalty for this job
    const weatherPenalty = getWeatherRiskPenalty(job.id);
    if (weatherPenalty > 0) {
      weatherAffectedJobs.push(job.id);
    }
    ...
    for (const entry of Array.from(crewState.entries())) {
      ...
      // Include weather penalty in scoring
      const score = driveMins * 1.5 + loadBalancingPenalty + weatherPenalty - zoneBonus;
  ...
  if (weatherBlockedJobs.length > 0) {
    warnings.push(`⚠️ ${weatherBlockedJobs.length} jobs blocked due to severe weather conditions (STOP risk)`);
  }
  if (weatherAffectedJobs.length > 0) {
    warnings.push(`⛈️ ${weatherAffectedJobs.length} jobs have weather risk - monitor conditions`);
  }

  const computeTimeMs = Date.now() - startTime;
  console.log(`[PlanningEngine] Computed plan in ${computeTimeMs}ms: ${assignments.length} crews, ${validJobs.length - unassignedJobs.length} assigned, ${unassignedJobs.length} unassigned, ${weatherBlockedJobs.length} weather-blocked`);
  ...
  return {
    ...
    overallUtilization,
    warnings,
    // Include weather metadata in result
    weatherData: {
      blockedJobs: weatherBlockedJobs,
      affectedJobs: weatherAffectedJobs
    }
  };
}
