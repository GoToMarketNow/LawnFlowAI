...
async function sendRatingPrompt(customerPhone: string, customerName: string): Promise<boolean> {
  try {
    ...
    const { twilioConnector } = await import('../../connectors/twilio-mock.js');
    const result = await twilioConnector.sendSMS(customerPhone, message);
    
    if (result.success) {
      console.log(`[ReviewManagementAgent] Rating prompt sent successfully`);
      return true;
    } else {
      console.error(`[ReviewManagementAgent] Failed to send rating prompt: ${result.error}`);
      return false;
    }
  } catch (error) {
...
async function sendReviewTextRequest(customerPhone: string, customerName: string): Promise<boolean> {
  try {
    ...
    if (result.success) {
      console.log(`[ReviewManagementAgent] Review text request sent successfully`);
      return true;
    } else {
      console.error(`[ReviewManagementAgent] Failed to send review text request: ${result.error}`);
      return false;
    }
  } catch (error) {
