...
async function sendSatisfactionPrompt(
  ...
  try {
    ...
    if (result.success) {
      console.log(`[PostJobQAAgent] Satisfaction prompt sent successfully, SID: ${result.sid}`);
      return true;
    } else {
      console.error(`[PostJobQAAgent] Failed to send satisfaction prompt: ${result.error}`);
      return false;
    }
  } catch (error) {
