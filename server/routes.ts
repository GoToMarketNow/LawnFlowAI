...
import mobileBindingRoutes from "./routes/mobile-binding-routes";
import testPaymentRoutes from "./routes/test-payment-routes";
...
export async function registerRoutes(
  ...
  // ============================================
  // Mobile Device Binding Routes
  // ============================================
  app.use("/api/mobile-binding", mobileBindingRoutes);
  ...
  app.get("/api/dashboard/metrics", apiLimiter, async (req, res) => {
    try {
      const businessId = (req.session as any).businessId || 1;
      const jobs = await storage.getJobsByBusinessId(businessId);
      const leads = await storage.getLeads(); // Leads are not currently business-isolated in schema, but should be eventually
      const quotes = jobs.filter((job) => job.estimatedPrice && job.estimatedPrice > 0);
      
      const activeJobs = jobs.filter(j => j.status === 'scheduled' || j.status === 'in_progress').length;
      const totalRevenue = jobs.filter(j => j.status === 'completed').reduce((sum, j) => sum + (j.estimatedPrice || 0), 0);
      ...
      res.json({
        ...
        activeJobs,
        activeJobsChange: 3, // Mock change
        avgMargin: 32, // Mock margin
        avgMarginChange: -2.1, // Mock change
        pendingQuotes,
        pendingQuotesChange: 4, // Mock change
      });
    } catch (error) {
  ...
  app.post("/api/leads/:id/convert-to-quote", apiLimiter, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      ...
      // Convert lead to a job with 'pending' status (acting as a quote)
      const job = await storage.createJob({
        ...
        businessId: (req.session as any).businessId || 1,
      });
  ...
  app.get("/api/crews", apiLimiter, async (req, res) => {
    try {
      const businessId = (req.session as any).businessId || 1;
  ...
  app.patch("/api/crews/:id", apiLimiter, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.delete("/api/crews/:id", apiLimiter, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      await storage.deleteCrew(id);
      res.json({ success: true });
    } catch (error) {
  ...
  app.get("/api/customers", apiLimiter, async (req, res) => {
    try {
      const businessId = (req.session as any).businessId || 1;
