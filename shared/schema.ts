...
export const mobileDeviceBindings = pgTable("mobile_device_bindings", {
  ...
  userId: integer("user_id").references(() => users.id).notNull(),
  businessId: integer("business_id").references(() => businessProfiles.id).notNull(),
  qrCodeToken: text("qr_code_token").notNull(), // JWT token from QR code
  ...
  bindingVerifiedAt: timestamp("binding_verified_at"),
  expiresAt: timestamp("expires_at").notNull(), // Token expiration (15 min)
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (table) => ({
  qrTokenIdx: uniqueIndex("mobile_device_bindings_qr_token_idx").on(table.qrCodeToken),
  userDeviceIdx: index("mobile_device_bindings_user_device_idx").on(table.userId, table.deviceId),
}));

// Onboarding Help Requests - track AI assistance usage
export const onboardingHelpRequests = pgTable("onboarding_help_requests", {
  id: serial("id").primaryKey(),
  sessionId: integer("session_id").references(() => onboardingSessions.id).notNull(),
  userId: integer("user_id").references(() => users.id).notNull(),
  step: text("step").notNull(), // Which onboarding step the help was requested for
  question: text("question").notNull(), // User's question
  aiResponse: text("ai_response").notNull(), // AI-generated response
  tokensUsed: integer("tokens_used"), // Track AI usage for metrics
  helpful: boolean("helpful"), // User feedback on response quality
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
});

// Test Payments - tracks onboarding test payments ($0.01 transactions)
export const testPayments = pgTable("test_payments", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id).notNull(),
  businessId: integer("business_id").references(() => businessProfiles.id).notNull(),
  sessionId: integer("session_id").references(() => onboardingSessions.id),
  paymentIntentId: text("payment_intent_id").notNull().unique(), // Stripe Payment Intent ID
  amount: integer("amount").notNull(), // Should always be 1 (cent)
  currency: text("currency").notNull().default("usd"),
  status: text("status").notNull().default("pending"), // pending, succeeded, failed
  stripeResponse: jsonb("stripe_response"), // Full Stripe response for debugging
  verifiedAt: timestamp("verified_at"),
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
});
...
export type MobileDeviceBinding = typeof mobileDeviceBindings.$inferSelect;
export type InsertMobileDeviceBinding = z.infer<typeof insertMobileDeviceBindingSchema>;

export const insertOnboardingHelpRequestSchema = createInsertSchema(onboardingHelpRequests).omit({
  id: true,
  createdAt: true,
});
export type OnboardingHelpRequest = typeof onboardingHelpRequests.$inferSelect;
export type InsertOnboardingHelpRequest = z.infer<typeof insertOnboardingHelpRequestSchema>;

export const insertTestPaymentSchema = createInsertSchema(testPayments).omit({
  id: true,
  createdAt: true,
});

export type TestPayment = typeof testPayments.$inferSelect;
export type InsertTestPayment = z.infer<typeof insertTestPaymentSchema>;
...
export const services = pgTable("services", {
  ...
  // Abstraction level for onboarding
  abstractionLevel: text("abstraction_level").default("SUBTYPE_SPECIFIC"), // CATEGORY_ONLY | SUBTYPE_SPECIFIC
